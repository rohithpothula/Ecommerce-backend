...
public class AuthenticationControllerTests {
  ...
  void testRegisterUserBlankUsernameFails() {
    RegistrationRequest request =
        new RegistrationRequest("", "email@example.com", "weak", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
  ...
  void testRegisterUserShortUsernameFails() {
    RegistrationRequest request =
        new RegistrationRequest("a", uniqueEmail, "weak", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
  ...
  void testRegisterUserLongUsernameFails() {
    RegistrationRequest request =
        new RegistrationRequest(
            "thisusernameisverylongandshouldnotbeaccepted", uniqueEmail, "weak", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
  ...
  void testRegisterUserInvalidUsernameFails() {
    RegistrationRequest request =
        new RegistrationRequest("user name", uniqueEmail, "weak", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
    ...
    assertTrue(errors.containsKey("user_name"));
    assertEquals(
        "User Name can only contain letters, numbers, and underscores", errors.get("user_name"));
  }

  @Test
  void testRegisterUserInvalidEmailFails() {
    RegistrationRequest request =
        new RegistrationRequest("username", "email", "weak", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
  ...
  void testRegisterUserShortPasswordFails() {
    RegistrationRequest request =
        new RegistrationRequest(uniqueUsername, uniqueEmail, "We@k1", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
    ...
    assertEquals(
        "Password is weak,it must contain at least 8 characters, include uppercase, lowercase, numbers, and special characters",
        errors.get("password"));
  }
  ...
  void testRegisterUserLongPasswordFails() {
    ...
    RegistrationRequest request =
        new RegistrationRequest(
            uniqueUsername, uniqueEmail, "Bbbbb@123".repeat(89), "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
  ...
  void testRegisterUserExistingUsernameFails() {
    RegistrationRequest request =
        new RegistrationRequest("username", uniqueEmail, "Password@123", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
  ...
  void testRegisterUserExistingEmailFails() {
    RegistrationRequest request =
        new RegistrationRequest(uniqueUsername, "abc@gmail.com", "Password@123", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    System.out.println(response);
    assertEquals(HttpStatus.CONFLICT, response.getStatusCode());
  ...
  @Test
  void testRegisterUserBlankEmailFails() {
  ...
  void testRegisterUserBlankPasswordFails() {
    RegistrationRequest request =
        new RegistrationRequest(uniqueUsername, uniqueEmail, "", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
  ...
  void testRegisterUserBlankFirstNameFails() {
    RegistrationRequest request =
        new RegistrationRequest(uniqueUsername, uniqueEmail, "password", "", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
  ...
  void testRegisterUserBlankLastNameFails() {
    RegistrationRequest request =
        new RegistrationRequest(uniqueUsername, uniqueEmail, "Password@123", "First", "");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map errors = response.getBody();
  ...
  void testRegisterUser_success() {
    RegistrationRequest request =
        new RegistrationRequest(uniqueUsername, uniqueEmail, "Password@123", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    System.out.println(response);
  ...
  void testRegisterUser_success_password_weak() {
    RegistrationRequest request =
        new RegistrationRequest(uniqueUsername, uniqueEmail, "password", "First", "Last");
    ResponseEntity<Map> response =
        testRestTemplate.postForEntity("/api/auth/registeruser", request, Map.class);
    assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    Map body = response.getBody();
