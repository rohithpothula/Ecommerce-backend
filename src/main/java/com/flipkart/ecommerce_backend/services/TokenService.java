...
/**
 * Service interface for handling JWT (Access Tokens) and Refresh Tokens. Defines operations for
 * creating, validating, extracting information from, and managing the lifecycle of tokens.
 */
public interface TokenService {

  /**
   * Generates a pair of tokens (access and refresh) for an authenticated user. Typically called
  ...
  /**
   * Generates a pair of tokens (access and refresh) directly from UserDetails. Useful during the
   * refresh token flow.
   *
  ...
  /**
   * Creates only the JWT Access Token for a given user. This might be used internally or for
   * specific flows if needed.
   *
  ...
  /**
   * Extracts the username (subject) from a given JWT access token. Validates the token internally
   * before extracting.
   *
  ...
  /**
   * Extracts all claims from a given JWT access token. Performs validation (signature, expiration)
   * before extracting.
   *
   * @param token The JWT access token string.
   * @return The Claims object containing the token payload.
   * @throws TokenException if the token is invalid (signature, format, expired, unsupported, empty
   *     claims).
   */
  ...
  /**
   * Verifies that the given Refresh Token has not expired. If it has expired, it should be removed
   * from the store, and an exception thrown.
   *
  ...
  /**
   * Deletes a Refresh Token from the persistent store using its token string. Used during logout or
   * when a refresh token is successfully used to get new tokens.
   *
