...
public class JwtService {
	...
	public String generateJWT(LocalUser localUser) {
		return JWT.create().withClaim(USERNAME_KEY,localUser.getUsername())
	...
	public String generateEmailJwt(LocalUser localUser) {
		return JWT.create().withClaim(EMAIL_VERIFICATION_KEY,localUser.getEmail())
				.withIssuedAt(new Date(System.currentTimeMillis()))
				.withExpiresAt(new Date(System.currentTimeMillis()+1000*expirySeconds))
				.withIssuer(jwtIssuer)
				.sign(algorithm);
	...
	public String generatePasswordResetJwt(LocalUser localUser) {
		return JWT.create().withClaim(PASSWORD_RESET_VERIFICATION_KEY, localUser.getEmail())
				.withIssuedAt(new Date(System.currentTimeMillis()))
