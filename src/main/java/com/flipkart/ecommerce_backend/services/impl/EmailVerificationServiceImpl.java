...
public class EmailVerificationServiceImpl implements EmailVerificationService {
  ...
  public String createAndPersistVerificationToken(LocalUser user) {
    if (user == null || user.getId() == null) {
      throw new IllegalArgumentException(
          "User must be persisted before creating a verification token.");
    }
    ...
    try {
      ...
      log.error(
          "Database error while saving email verification token for user {}: {}",
          user.getUsername(),
          e.getMessage());
  ...
  @Override
  @Transactional
  public LocalUser verifyEmailWithToken(String token)
    ...
    if (verificationToken.isExpired()) {
      log.warn(
          "Email verification failed: Token expired for user {}",
    ...
    if (user.isEmailVerified()) {
      ...
      throw new UserVerificationTokenAlreadyVerifiedException(
          "User email (" + user.getEmail() + ") is already verified.");
    }
    ...
    try {
      ...
      log.error(
          "Database error while marking user {} as verified: {}",
          user.getUsername(),
          e.getMessage());
