...
public class AddressServiceImpl implements AddressService {
  ...
  public Page<AddressDTO> getAddressesForUser(UUID userId, Pageable pageable) {
    ...
    log.debug(
        "Found {} addresses on page {} for user ID: {}",
  ...
  public AddressDTO getAddressById(UUID addressId, UUID userId) {
    ...
    if (userId != null && !address.getLocalUser().getId().equals(userId)) {
      log.warn(
          "User {} attempted to access address {} owned by user {}",
          userId,
          addressId,
          address.getLocalUser().getId());
      throw new UnauthorizedAccessException(
          "User does not have permission to access this address.");
    }
  ...
  @Override
  @Transactional
  public AddressDTO updateAddressForUser(
      UUID addressId, UUID userId, AddressRequestDTO addressRequestDTO) {
    log.info("Updating address ID: {} for user ID: {}", addressId, userId);
    Address address = findAddressByIdInternal(addressId);

    if (!address.getLocalUser().getId().equals(userId)) {
      log.warn(
          "User {} attempted to update address {} owned by user {}",
          userId,
          addressId,
          address.getLocalUser().getId());
      throw new UnauthorizedAccessException(
  ...
  public void deleteAddressForUser(UUID addressId, UUID userId) {
    ...
    if (!address.getLocalUser().getId().equals(userId)) {
      log.warn(
          "User {} attempted to delete address {} owned by user {}",
          userId,
          addressId,
          address.getLocalUser().getId());
      throw new UnauthorizedAccessException(
