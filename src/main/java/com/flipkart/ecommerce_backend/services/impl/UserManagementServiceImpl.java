...
public class UserManagementServiceImpl implements UserManagementService {
    ...
    public UserDto registerUser(RegistrationRequest registrationRequest) {
        ...
        userRepository.findByUsernameIgnoreCase(registrationRequest.user_name())
                .ifPresent(u -> { throw new UserAlreadyExistsException("Username '" + registrationRequest.user_name() + "' already exists."); });
        userRepository.findByEmailIgnoreCase(registrationRequest.email())
                .ifPresent(u -> { throw new UserAlreadyExistsException("Email '" + registrationRequest.email() + "' already exists."); });
        ...
        Role userRole = roleRepository.findByName(ERole.ROLE_USER) // Use enum name() for lookup if repo expects String
                .orElseThrow(() -> {
        ...
        log.info("User entity registered successfully (pending verification) with ID: {}", savedUser.getId());
    ...
    public UserDto findUserDTOByUsernameOrEmail(String usernameOrEmail) {
        ...
        LocalUser user = userRepository.findByUsernameIgnoreCase(usernameOrEmail)
        ...
        if (request.email() != null && !request.email().equalsIgnoreCase(user.getEmail())) {
            userRepository.findByEmailIgnoreCase(request.email()).ifPresent(existingUser -> {
                if (!existingUser.getId().equals(userId)) {
                    throw new UserAlreadyExistsException("Email '" + request.email() + "' is already in use by another user.");
                }
            });
    ...
    public Page<UserDto> getUsersByRole(ERole roleName, Pageable pageable) {
        log.info("Fetching users by role '{}' - page: {}, size: {}", roleName, pageable.getPageNumber(), pageable.getPageSize());
