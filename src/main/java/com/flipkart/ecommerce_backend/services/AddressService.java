...
public interface AddressService {
   ...
   * @throws com.flipkart.ecommerce_backend.exception.user.UserNotFoundException if the user doesn't
  ...
  /**
   * Retrieves a specific address by its ID. Includes an optional check to ensure the address
   * belongs to the specified user if needed.
   *
   * @param addressId The ID of the address to retrieve.
   * @param userId (Optional) The ID of the user who should own the address. If provided, verifies
   *     ownership.
  ...
  /**
   * Updates an existing address. Includes an optional check to ensure the address belongs to the
   * specified user before updating.
   *
   ...
   * @throws com.flipkart.ecommerce_backend.exception.DatabaseException on persistence errors.
   */
  AddressDTO updateAddressForUser(
      UUID addressId, UUID userId, @Valid AddressRequestDTO addressRequestDTO);

  /**
   * Deletes an address by its ID. Includes an optional check to ensure the address belongs to the
   * specified user before deleting.
   *
