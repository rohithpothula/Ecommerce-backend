...
public interface EmailVerificationService {
  ...
  /**
   * Creates a unique email verification token for the given user, persists it (potentially removing
   * old ones), and returns the token string.
   *
  ...
  /**
   * Verifies the user's email using the provided token. Checks if the token exists, is not expired,
   * and belongs to an unverified user. If valid, marks the user as verified and deletes the token.
   *
   ...
   * @throws UserVerificationTokenAlreadyVerifiedException if the user associated with the token is
   *     already verified.
   * @throws com.flipkart.ecommerce_backend.exception.DatabaseException on persistence errors.
   */
  LocalUser verifyEmailWithToken(String token)
