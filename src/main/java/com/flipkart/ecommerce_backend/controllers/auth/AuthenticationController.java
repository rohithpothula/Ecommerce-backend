...
public class AuthenticationController {
   ...
   */
    ...
    public ResponseEntity<UserDto> registerUser(@Valid @RequestBody RegistrationRequest registrationRequest) throws RoleNotFoundException {
        UserDto savedUserDTO = userManagementService.registerUser(registrationRequest);

        try {
            LocalUser registeredUser = userManagementService.findEntityByUsernameOrEmail(savedUserDTO.username());
            String verificationToken = emailVerificationService.createAndPersistVerificationToken(registeredUser);
            ...
            log.error("Error during post-registration email verification steps for {}: {}",
                    savedUserDTO.username(), e.getMessage(), e);
        }
    ...
    @PostMapping("/login")
    public ResponseEntity<GenericResponseBodyDto> loginUser(@Valid @RequestBody LoginRequest loginRequest) {
        ...
        if (authentication == null || !(authentication.getPrincipal() instanceof LocalUser)) {
             log.error("Authentication failed or principal is not LocalUser after successful login service call for {}", loginRequest.username());
             throw new IllegalStateException("Authentication principal is not available or not of expected type after login.");
        }
    ...
    @PostMapping("/logout")
    public ResponseEntity<GenericResponseBodyDto> logoutUser(@Valid @RequestBody RefreshTokenRequest refreshTokenRequest) {
    ...
    @GetMapping("/me")
    public ResponseEntity<UserDto> getLoggedInUserProfile(@AuthenticationPrincipal UserDetails userDetails) {
        if (userDetails == null) {
