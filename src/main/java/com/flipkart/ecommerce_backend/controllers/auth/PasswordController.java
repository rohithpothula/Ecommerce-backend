...
public class PasswordController {
  /** /forgot - POST /reset - POST /change - POST */
  ...
  public ResponseEntity<GenericResponseBodyDto> forgotPassword(@RequestParam @Email String email) {
    ...
    try {
      ...
      response.setDetails(
          Map.of("email", email, "nextStep", "Check your email for the password reset link"));
      ...
      response.setTimestamp(LocalDateTime.now().toString());
      response.setDetails(Map.of("email", email, "error", e.getMessage()));
  ...
  public ResponseEntity<GenericResponseBodyDto> resetPassword(
    ...
    log.info(
        "Received request to reset password with token: {}",
        resetPasswordRequest.token()); // Avoid logging the password
